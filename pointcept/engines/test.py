...
class KeyDetTester(TesterBase):
    def test(self):
        ...
        # New meters per category, as in KeyPointNet evaluation
        fp_meter = CategoryAverageMeter()
        fn_meter = CategoryAverageMeter()
        num_keypoints_meter = CategoryAverageMeter()
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                ...
                with torch.no_grad():
                    pred = self.model(data_dict)["seg_logits"]
            ...
            # New part, evaluation as in KeypointNet
            fp, fn, num_keypoints = fp_fn_geodist(pred, segment, coord.cpu().numpy(), dist_thresh=0.01, iou_thresh=0.1)

            fp_meter.update(fp, category=data_category)
            fn_meter.update(fn, category=data_category)
            num_keypoints_meter.update(num_keypoints, category=data_category)

            record[data_name] = dict(
                intersection=intersection, union=union, target=target, fp=fp, fn=fn, num_keypoints=num_keypoints,
                category=data_category
            )
            """
            if data_category in iou_dict.keys():
                iou_dict[data_category]['fn'] += fn
                iou_dict[data_category]['fp'] += fp
                iou_dict[data_category]['n'] += n
            else:
                iou_dict[data_category] = {'fn': fn, 'fp': fp, 'n': n}
            """
        logger.info("Syncing ...")
        comm.synchronize()
        record_sync = comm.gather(record, dst=0)
        ...
        if comm.is_main_process():
            ...
            # TODO: Check if it is possible to do like they do for segmentation classes,
            #  i.e. for i in range(self.cfg.data.test.class_id2names): and iou on ndarray(num_category, )
            # New meters per category, as in KeyPointNet evaluation
            ...
            """
            for cat, values in iou_dict.items():
                logger.info(
                    "Category_{cat} Result: mIoU with distance threshold 0.01: {iou:.4f}".format(
                        cat=cat,
                        iou=(values['n'] - values['fn']) / np.maximum(values['n'] + values['fp'], np.finfo(np.float64).eps)
                    )
                )
            """
            logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
